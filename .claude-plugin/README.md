...
General developer skills and agents for coding agents — a [Claude Code](https://docs.anthropic.com/en/docs/claude-code) plugin providing 10 composable skills for commit workflows, feature development, codebase analysis, and documentation generation.

## Installation

```bash
claude plugin add sequenzia/agent-tools --path plugins/dev-tools
```
...
The plugin includes a [Context7](https://context7.com) MCP server (configured in `.mcp.json`) for fetching up-to-date external library documentation at runtime. Skills that interact with third-party libraries use Context7 to resolve library IDs and query current API docs.
