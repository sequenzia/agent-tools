 ...
 */
export function checkConflict(filePath, lastReadMtimeMs) {
    if (!fs.existsSync(filePath)) {
        throw new Error(`Conflict: task file was removed externally: ${filePath}`);
    }
    const currentMtime = getMtimeMs(filePath);
    if (currentMtime !== lastReadMtimeMs) {
        throw new Error(`Conflict: file was modified externally since last read. ` +
 ...
 */
export function atomicWrite(targetPath, contents) {
    ...
    try {
        ...
        throw new Error(`Failed to rename temp file to ${targetPath}: ${e.message}`);
    }
}
/**
 ...
 */
export function resolveSpecPath(projectPath, specPath) {
