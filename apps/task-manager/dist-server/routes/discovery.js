...
export function createDiscoveryRouter(broadcast) {
    const router = Router();
    /** POST /api/discovery/scan — Scan for projects. */
    router.post("/scan", (req, res) => {
        const { rootPaths, maxDepth } = req.body;
        ...
        for (const rootStr of rootPaths) {
            ...
            const queue = [[rootStr, 0]];
            while (queue.length > 0) {
                if (dirsScanned >= MAX_DIRS_SCANNED) {
                    warnings.push(`Safety limit reached: scanned ${MAX_DIRS_SCANNED} directories, stopping`);
                    break;
                }
                ...
                if (currentDepth > 0 && shouldSkipDir(dirName))
                    continue;
