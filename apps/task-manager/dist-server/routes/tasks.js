...
function scanStatusDir(statusDir) {
    const results = [];
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            results.push({
                type: "error",
                file_path: groupDir,
                error: `Failed to read group directory: ${e.message}`,
            });
            continue;
        }
 ...
 */
function validateTaskJson(task, expectedStatus) {
    ...
    if (!("description" in task) || typeof task.description !== "string") {
        throw new Error(!("description" in task)
    ...
    if (!("status" in task) || typeof task.status !== "string") {
        throw new Error(!("status" in task)
            ? "Validation failed: missing required field 'status'"
            : "Validation failed: field 'status' must be a string");
    }
    const status = task.status;
    if (!VALID_STATUSES.includes(status)) {
        throw new Error(`Validation failed: field 'status' has invalid value '${status}'. Expected one of: ${VALID_STATUSES.join(", ")}`);
    }
    if (expectedStatus && status !== expectedStatus) {
        throw new Error(`Validation failed: field 'status' is '${status}' but target directory is '${expectedStatus}'. Status must match the target directory.`);
    }
    ...
    if (metadata && typeof metadata === "object") {
        if (metadata.priority !== undefined) {
            ...
            if (!VALID_PRIORITIES.includes(metadata.priority)) {
                throw new Error(`Validation failed: field 'metadata.priority' has invalid value '${metadata.priority}'. Expected one of: ${VALID_PRIORITIES.join(", ")}`);
            }
        }
        if (metadata.complexity !== undefined) {
            ...
            if (!VALID_COMPLEXITIES.includes(metadata.complexity)) {
                throw new Error(`Validation failed: field 'metadata.complexity' has invalid value '${metadata.complexity}'. Expected one of: ${VALID_COMPLEXITIES.join(", ")}`);
            }
        }
    }
    // Validate timestamps
    if (task.created_at !== undefined) {
        ...
        if (!isValidIso8601(task.created_at)) {
            throw new Error(`Validation failed: field 'created_at' has invalid ISO 8601 timestamp '${task.created_at}'`);
        }
    }
    if (task.updated_at !== undefined) {
        ...
        if (!isValidIso8601(task.updated_at)) {
            throw new Error(`Validation failed: field 'updated_at' has invalid ISO 8601 timestamp '${task.updated_at}'`);
        }
    }
    // Validate blocked_by
    if (task.blocked_by !== undefined && task.blocked_by !== null) {
        ...
        for (let i = 0; i < task.blocked_by.length; i++) {
            ...
            if (typeof item !== "string" && typeof item !== "number") {
                throw new Error(`Validation failed: field 'blocked_by[${i}]' must be a string or number, got: ${JSON.stringify(item)}`);
            }
        }
    }
}
 ...
 */
function validateBlockedByReferences(task, tasksBase) {
    ...
    for (const item of blockedBy) {
        ...
        if (!knownIds.has(idStr)) {
            throw new Error(`Validation failed: blocked_by references unknown task ID '${idStr}'. No task file found with this ID in any status directory.`);
        }
    }
}
// --- Routes ---
...
router.get("/", (req, res) => {
    ...
    for (const statusDirName of STATUS_DIRS) {
        ...
        const key = normalizeStatus(statusDirName);
        result[key] = tasks;
    }
    res.json(result);
});
...
router.get("/manifest", (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: `Failed to read manifest: ${e.message}` });
    }
});
/** POST /api/tasks/move — Move a task to a new status directory. */
router.post("/move", (req, res) => {
    const { filePath, newStatus, lastReadMtimeMs } = req.body;
    ...
    try {
        ...
        const contents = fs.readFileSync(filePath, "utf-8");
        const taskValue = JSON.parse(contents);
...
router.post("/update", (req, res) => {
    const { filePath, fields, lastReadMtimeMs } = req.body;
    if (!filePath || !fields) {
