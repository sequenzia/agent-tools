...
function liveSessionPath(projectPath: string): string {
...
function hasLockFile(sessionPath: string): boolean {
  return fs.existsSync(path.join(sessionPath, ".lock"));
}

/** List available session files in a session directory. */
function listAvailableFiles(sessionPath: string): string[] {
  const files: string[] = [];
...
export function classifySessionStatus(projectPath: string): SessionStatus {
  const sessionPath = liveSessionPath(projectPath);

  try {
    const stat = fs.statSync(sessionPath);
    if (!stat.isDirectory()) return "inactive";
  } catch {
    return "inactive";
  }
  ...
  if (hasSummary) {
    try {
      const content = fs.readFileSync(
        path.join(sessionPath, "session_summary.md"),
        "utf-8",
      );
      summary = parseSessionSummary(content);
    } catch (e) {
      error = `Failed to read session_summary.md: ${(e as Error).message}`;
    }
  }
...
router.get("/live", (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: `Permission denied on session directory: ${(e as Error).message}`,
    });
    return;
  }

  const hasLock = hasLockFile(sessionPath);
  const status: SessionStatus = hasLock ? "active" : "interrupted";
  const availableFiles = listAvailableFiles(sessionPath);
...
router.get("/file", (req, res) => {
  ...
  if (!fs.existsSync(filePath)) {
    const result: SessionFileResult = {
      filename,
      content: null,
      error: null,
      exists: false,
...
router.get("/results", (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: `Failed to read session directory: ${(e as Error).message}`,
    });
  }
});
...
router.get("/archived", (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: `Failed to read sessions directory: ${(e as Error).message}`,
    });
  }
});
