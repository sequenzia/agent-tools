...
function analysisPathFor(specPath: string): string {
...
router.get("/analysis", (req, res) => {
  ...
  try {
    ...
    const result: SpecAnalysisCheck = {
      exists,
...
router.get("/lifecycle", (req, res) => {
  const projectPath = req.query.projectPath as string;
  const specPath = req.query.specPath as string;
  const taskGroup = req.query.taskGroup as string | undefined;
  ...
  try {
    ...
    const completedStages: SpecLifecycleStage[] = ["created"];
    ...
    const liveSessionPath = path.join(
      projectPath,
      ".agents",
      "sessions",
      "__live_session__",
    ...
    const specModifiedAfterTasks =
      hasTasks && latestTaskMtime > 0
        ? getMtimeMs(resolved) > latestTaskMtime
        : false;

    const currentStage =
      completedStages[completedStages.length - 1] ?? "created";
