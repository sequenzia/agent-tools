...
1. **Load report template:**
   - Read `resources/report-template.md`
...
2. **Execute selected actions in the following fixed order:**
   ...
   **Step 2: Present severity-ranked item list**
   ...
   - Load reference template from `resources/actionable-insights-template.md`
   ...
   **Step 3: Process each selected item in priority order (High → Medium → Low)**
   ...
   2. **Plan the fix:**
      ...
      - Complex (architectural): Read `../architecture-patterns/SKILL.md` for guidance, then design the fix using the blueprint structure from that skill
      - Complex (needs investigation): Use Glob/Grep/Read to investigate the codebase before proposing changes
