...
# Example: poll-for-results.sh .agents/sessions/__live_session__ task-001 task-002 task-003
#
# Checks for result-{id}.md files every INTERVAL seconds for up to
...
while [ "$ELAPSED" -lt "$TOTAL_TIMEOUT" ]; do
  ...
  for ID in $EXPECTED_IDS; do
    TOTAL=$((TOTAL + 1))
    if [ -f "$SESSION_DIR/result-$ID.md" ]; then
