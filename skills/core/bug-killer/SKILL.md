...
name: bug-killer
description: >-
  Systematic, hypothesis-driven debugging workflow with triage-based track
  routing. Use when asked to "fix this bug", "debug this", "why is this
  failing", "this is broken", "investigate this error", "track down this
  issue", or any debugging situation. Supports --deep flag to force full
  investigation.
metadata:
...
Extract from the user's input and conversation context:
...
|----------|---------------|
| Python | `Read references/python-debugging.md` |
| TypeScript / JavaScript | `Read references/typescript-debugging.md` |
| Other / Multiple | `Read references/general-debugging.md` |
...
2. **Launch code-explorer subagents:**
   ...
   - Focus area: [specific area for this subagent]
   ...
   Launch subagents in parallel for independent focus areas.
...
2. **Launch bug-investigator subagents:**
   ...
   Each subagent should report findings with verdict (confirmed/rejected/inconclusive),
   evidence, and recommendations.

   Launch subagents in parallel when they test independent hypotheses.
...
2. **Check for related issues:**
   ...
   - Ask the user: "Fix all related instances now?" / "Fix only the reported bug" / "Create tasks for related fixes"
...
```
Read ../project-learnings/SKILL.md
```
...
Ask the user which next step they prefer:
...
|--------|-------------|------------|
| Investigation | Read error location + 1-2 callers | 2-3 code-explorer subagents in parallel |
| Hypotheses | Minimum 1 | Minimum 2-3 |
| Root cause testing | Manual verification | 1-3 bug-investigator subagents in parallel |
