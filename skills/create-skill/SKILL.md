...
After rendering and validation, prompt the user for the output location using `AskUserQuestion`.
...
Before writing the file, check whether the target directory exists. If it does not exist, create the full directory path using `Bash` with `mkdir -p`. If directory creation fails (permissions, invalid path), report the error and re-prompt the user for a different path via `AskUserQuestion`.

#### Overwrite Protection

Before writing, check if a file already exists at the target path. If it does, prompt the user via `AskUserQuestion`:
