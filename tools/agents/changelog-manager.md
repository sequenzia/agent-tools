...
- `Fixed bug` (does not explain what)
...
2. Create category headings if they do not exist
...
- **File system access**: Read and modify CHANGELOG.md
- **Search**: Search for files by name patterns and search within file contents
- **Shell execution**: Run shell commands (git operations, gh CLI)
- **User interaction**: Prompt the user to approve or edit changelog entries

### Adaptation Guidance

- This agent is typically spawned by the **release** skill during Step 5 (Changelog Update Check).
- The gh CLI integration (Step 5) is optional — the agent works without it using git data alone.
- For monorepos, use the path filter feature to scope changelog entries to specific sub-projects.
