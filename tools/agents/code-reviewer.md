...
When your task is complete, report your findings to the coordinator. When another worker messages you with a follow-up question:
...
- **Search**: Search for files by name patterns and search within file contents to verify context

### Adaptation Guidance

- This agent is read-only — it reviews code but does not modify it.
- It is typically spawned by the **feature-dev** skill during Phase 6 (Quality Review).
- If the host environment does not support parallel workers, the review can be performed sequentially with different focus areas in each pass.
