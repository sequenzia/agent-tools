...
1. **Accuracy first** — Always read the actual source code before documenting. Never guess at function signatures, parameter types, or behavior. If exploration findings are incomplete, search for and read files to verify.
...
- **File system access**: Read source files to verify documentation accuracy
- **Search**: Search for files by name patterns and search within file contents
- **Shell execution**: Run shell commands if needed for verification

### Adaptation Guidance

- This agent is typically spawned by the **docs-manager** skill during Phase 5 (Documentation Generation).
- It depends on the **technical-diagrams** skill (from the core-tools package) for Mermaid diagram styling conventions. If unavailable, use standard Mermaid syntax with dark text colors on nodes.
- Multiple instances can run in parallel for independent documentation pages, with sequential execution for dependent pages.
