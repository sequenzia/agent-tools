#!/bin/bash
# resolve-cross-plugins.sh — Session-start hook for cross-plugin dependency resolution.
#
# Creates short-name symlinks so ../{short-name}/ path references work in
# cached/installed plugin environments. In the local monorepo, ../short-name/
# resolves naturally to sibling directories. In the cache, the extra version
# subdirectory and org-prefixed names break that convention. This script
# bridges the gap by creating symlinks at the plugin-name directory level.
#
# Usage: bash resolve-cross-plugins.sh <PLUGIN_ROOT>
