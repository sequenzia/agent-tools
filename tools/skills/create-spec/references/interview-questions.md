...
2. **Response approach**:
   ...
   - Present options using the `question` tool with clear trade-offs
3. **Example flow**:
   ```
   User: "I'm not sure what authentication approach to use"

   Agent: [Detects auth trigger + uncertainty]
   Agent: [Offers inline insight via question tool]
...
- If exploration was performed, use the findings to:
